...
import { addRecordApi, openRecordApi } from "../../helpers/api/AddRecordApi";
...
test(
  ...
  async ({ request }) => {
    ...
    await test.step("Add a Document record via the WebAPI", async () => {
      const response = await addRecordApi(users.mgrDoc, "Documents_p", docPayload);
    ...
    await test.step("Validate that a user without permissions cannot view the Document record", async () => {      
      const response = await openRecordApi(users["31S1002QM"], "Documents_p", id);
    ...
    await test.step("Validate that a user with permissions can view the Document record", async () => {
      ...
      const response = await openRecordApi(users.demo, "Documents_p", id);
