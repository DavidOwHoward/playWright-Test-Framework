...
export class NewMfgDocumentRequests_p extends DetailsPageBase {
  get ITARRestricted() {
  ...
  get Site() {
    ...
    const { root, section } = this.fieldRootInSection(`SysSite_f`, `General`);

    return new ScrField(this.page, root, async () => section.openAndWait(root));
  }

  get NewManufacturingDocument() {

    const { root, section } = this.fieldRootInSection(`NewManufacturingDocument_f`, `General`);
