...
export class HomeTopToolBar {
  ...
  constructor(page: Page) {
    ...
    this.walkMeDisabled = page
      .locator("EQMS-GUIDE-ME-DIALOG")
      .getByText("Could not establish connection to Guide me server.");

    this.closeDisabledWalkMe = page
      .locator('[role="dialog"]:has(mat-icon:text("close"))')
      .getByText("close");
    ...
    this.globalClose = page
      .locator("eqms-global-search")
    ...
    this.unsavedChanges = new ConfirmationDialog(page, "Unsaved Changes");
    this.loginHeading = page.getByRole("heading", { name: "Sign in to EQMS" });
  }
  ...
  async openDocLibrary() {
    ...
    await expect(
      this.page.getByRole("menuitem", { name: /about/i }),
  ...
  async openWalkMe(): Promise<"opened" | "disabled"> {
    ...
    const result = await Promise.race([
      this.walkMeFrame
        ...
        .then(() => "Walk Me opened" as const),
      this.walkMeDisabled
        .waitFor({ state: "visible", timeout: 5000 })
        .then(() => "Walk Me is disabled" as const),
    ]);
  ...
  async openSettings() {
    ...
    await expect(
      this.page
        ...
        .getByText("Show User Name"),
    ).toBeVisible();
  }

  async userLogout() {
    await this.profile.click();
    await expect(this.logout).toBeVisible({ timeout: 2000 });
    await this.logout.click();
    await expect(
      this.page.url(),
      "Check to see if user is redirected to login page",
    ).toMatch(/authentication/i);
  }
}
